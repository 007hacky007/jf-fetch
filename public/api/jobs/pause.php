...
if (!$isAdmin && (int) $job['user_id'] !== (int) $user['id']) {
    Http::error(403, 'Insufficient permissions to pause this job.');
    exit;
}

if (!in_array($job['status'], ['starting', 'downloading'], true)) {
    Http::error(422, 'Only active jobs can be paused.');
    exit;
}
...
try {
    ...
    Http::error(502, 'Failed to pause aria2 transfer: ' . $exception->getMessage());
    exit;
}
...
if ($updated === null) {
    Http::error(500, 'Job paused but failed to reload updated data.');
    exit;
}

Http::json(200, ['data' => Jobs::format($updated, $isAdmin)]);

Audit::record((int) $user['id'], 'job.paused', 'job', $jobId, [
    'aria2_gid' => $aria2Gid,
]);
Events::notify((int) $user['id'], $jobId, 'job.paused', [
